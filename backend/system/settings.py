...
class Settings:
    def __init__(self):
        ...
        self.rw_props = [
            ...
            'autogenerated_access_points_ssid',
        ]

        self.on_update = []
        self.reload()
        if not self.autogenerated_access_points_ssid:
            self.update({ 'autogenerated_access_points_ssid': self.__autogenerated_access_point_ssid() });
    ...
    @property
    def autogenerated_access_points_ssid(self):
        return self.json_map.get('autogenerated_access_points_ssid')
    ...
    def update(self, new_data):
        ...
        if 'server_name' in new_data:
            new_data['autogenerated_access_points_ssid'] = self.__autogenerated_access_point_ssid(server_name=new_data['server_name'])
    ...
    def __autogenerated_access_point_ssid(self, server_name=None):
        ssid_name = 'AstroPhoto-Plus'
        if not server_name:
            server_name = self.server_name

        if server_name:
            if re.match('astrophoto.{0,1}plus', server_name.lower()):
                ssid_name = server_name
            else:
                ssid_name = 'AstroPhoto-Plus-{}'.format(server_name)
        random.seed(time.time())
        ssid = '{}-{}'.format(ssid_name, random.randrange(1000, 10000))
        return ssid
